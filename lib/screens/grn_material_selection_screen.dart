...
class GrnMaterialSelectionScreen extends StatefulWidget {
  ...
  @override
  State<GrnMaterialSelectionScreen> createState() => _GrnMaterialSelectionScreenState();
}

class _GrnMaterialSelectionScreenState extends State<GrnMaterialSelectionScreen> {
      ...
      if (response != null && response.status == 1) {
        setState(() {
          ...
          for (var material in _materials) {
            _quantityControllers[material.id] = TextEditingController(text: '1');
  ...
  List<Map<String, dynamic>> _getSelectedMaterials() {
    ...
    for (var entry in _selectedMaterials.entries) {
      if (entry.value) {
        ...
        final quantity = int.tryParse(_quantityControllers[entry.key]?.text ?? '1') ?? 1;
  ...
  void _proceedToRecordGrn() {
    ...
    if (selectedMaterials.isEmpty) {
      SnackBarUtils.showWarning(context, message: 'Please select at least one material');
      return;
    }
                ...
                : _filteredMaterials.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isSelected = _selectedMaterials[material.id] ?? false;
                          final quantityController = _quantityControllers[material.id];
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: TextField(
                                          ...
                                          onChanged: (value) => _updateQuantity(material.id, value),
                                        ),
                                      ),
                                      SizedBox(width: 6),
                                      Expanded(
